...
const ProfileCard = ({ user }) => {
  const { instituteName, directorName, email, address, username, directorPhoto } = user;
  ...
  return (
    <div className="bg-gray-800 bg-opacity-80 backdrop-blur-sm rounded-lg shadow-lg p-6 max-w-sm mx-auto my-8 border border-gray-700 w-full">
      <div className="flex flex-col items-center text-center">
        ...
        {directorPhoto ? (
          <img
            src={directorPhoto}
            alt={directorName}
            className="w-24 h-24 rounded-2xl object-cover mb-4 border-2 border-indigo-500"
          />
        ) : (
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-70 flex items-center justify-center bg-gray-700 bg-opacity-70 backdrop-blur-sm p-4 rounded-2xl">
          <div className="bg-gray-800 p-8 rounded-xl shadow-2xl border border-gray-700 max-w-lg w-full relative">
            ...
            {directorPhoto && (
              <div className="flex justify-center mb-4">
                <img src={directorPhoto} alt={directorName} className="w-32 h-32 rounded-full object-cover border-2 border-indigo-500" />
              </div>
            )}
            ...
            <div className="space-y-4 text-gray-300">
              <p><span className="font-semibold text-gray-100">Institute:</span> {instituteName}</p>
              <p><span className="font-semibold text-gray-100">Director:</span> {directorName}</p>
              <p><span className="font-semibold text-gray-100">Address:</span> {address}</p>
            ...
            <div className="mt-6 text-center">
              <button
                onClick={() => setIsModalOpen(false)}
                className="bg-pink-500 hover:bg-pink-600 text-gray-100 font-bold py-2 px-6 rounded-full transition-colors duration-400"
              >
                Close
              </button>
            </div>
